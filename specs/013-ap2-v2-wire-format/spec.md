...
`new CredentAgent({ mandateSigningKey })` accepts a host-supplied private P-256 JWK. The
option is deliberately NOT called `signingKey`: that name is already the ceremony seams'
symmetric HMAC secret for challenge tokens, and two different keys sharing one name is how
a config mistake becomes silent. When absent, the gate
...
repo. Hand-rolling RFC 9901 disclosure handling was the alternative and is not worth the
defect surface.

The ES256 layer underneath is `node:crypto`, NOT `jose`. `jose` stays a dependency for the
existing rails (the dc-payment request, the credential gate) but the AP2 module does not
reach for it: `node:crypto`'s `sign`/`verify` with `dsaEncoding: "ieee-p1363"` emit exactly
the raw r‖s encoding JWS requires (node's EC default is DER, which no JWS verifier accepts),
and they are **synchronous**. That matters because `mount()` is synchronous — resolving the
key on a promise would let the routes attach some ticks before the public key reached
`app.locals`, a race in the middle of a security check.
