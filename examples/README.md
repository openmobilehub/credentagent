...
- [`grants-approve/`](grants-approve/) — the spending-grant approve page (`grant.approveUrl`) in every state: before and after the human proves their age, the loyalty discount applied, and the terminal state. Tap either wallet button for the live ceremony
