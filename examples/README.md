...
- [`grants-approve/`](grants-approve/) — the spending-grant APPROVE PAGE (`grant.approveUrl`) in every state: an age-restricted scope before and after the human proves their age, the loyalty discount applied, an unrestricted scope, and the terminal state. Tap either wallet button for the live ceremony
