...
| `--mpzpass` | also write `dpc.mpzpass`, the container the wallet imports — into `--keys`, because it carries the holder's private key |
| `--holder <name>` | cardholder name on the credential |
| `--out <dir>` | publishable artifacts — `dpc.sdjwt`, `dpc.json` (default `../out`, **tracked by git**) |
| `--keys <dir>` | anything carrying a private key (default `../keys`, **gitignored**) |

### Where the output lands, and why it is split

`../out` is a tracked directory: a private key written there is one `git add tools/demo-pki/out`
away from being in a commit, and a pushed key cannot be un-pushed by deleting the file. So
nothing private defaults into it. Generated keys and `dpc.mpzpass` go to `../keys` instead —
the directory `gen-pki.sh` already uses, covered by `../.gitignore` under *"Private keys never
leave this machine"*. `out/*.jwk`, `out/*.pem` and `out/dpc.mpzpass` are ignored as well, so
pointing `--keys` at `out/` still cannot produce a commit.
